...
module.exports = {
    ...
    description: 'Generate Actors from the prompt',
    static: true,
    inputs: {
        prompt: {
            type: "string",
    ...
    fn: async function (inputs, env) {
        ...
        try {
            ...
            for (let i in models) {
                let model = models[i];
                let item = note.addToItems({type: 'AActor', json: model});
                item.note = note;
                items.push(item);
            }
            note.save();
