...
module.exports = {
    ...
    description: 'Generate Scenarios from the prompt',
    static: true,
    inputs: {
        prompt: {
            type: "string",
    ...
    fn: async function (inputs, env) {
        const format = AScenario.schema();

        let messages = [];
        let ujson = AUseCase.toPrompt();
        let sjson = AScenario.toPrompt();

        let content = `Use the following usecase definitions for analysis of the user prompt: ${ujson}`;
        messages.push({role: 'system', content: content});

        content = `Use the following scenarios definitions for analysis and check for duplicates of the user prompt: ${sjson}`;
        messages.push({role: 'system', content: content});

        // Get the current documentation. Add it as system information.
        messages.push({role: 'system',
            content: `From the user prompt identify and generate scenarios or scenarios to update  using the following as a` +
        ...
        try {
            ...
            for (let i in models) {
                let model = models[i];
                let item = note.addToItems({type: 'AScenario', json: model});
                item.note = note;
                items.push(item);
            }
            note.save();
