...
module.exports = {
    ...
    fn: async function (obj, inputs, env) {
        ...
        let messages = [];
        let note = ANote.construct({prompt: inputs.prompt});

        AEvent.emit({event: "actor.generate.started", data: {message: "Generating Actors from Notes"}});
        ...
        messages.push({
            role: 'user', content: "Based on the following information generate any new actors for the system. " +
                ...
                `and all lowercase and a wellknown abbreviation for the actorName. Here is the following information: ${inputs.prompt}`
        });

        let actors = await AIHelper.askForCode(messages);
        for (let i in actors) {
            let actor = actors[i];
            note.addToItems({type: 'AActor', json: actor});
        }
        note.save();
        ...
        AEvent.emit({event: "actor.generate.completed", data: {message: `Generated ${actors.length} Actors from Notes`}});
        return obj;
    }
};
